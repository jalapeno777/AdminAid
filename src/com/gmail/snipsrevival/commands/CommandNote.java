...
public class CommandNote implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) { 
		...
		if(args.length == 0) {
			...
			if(args[0].equalsIgnoreCase("add") && sender.hasPermission("adminaid.note.add")) {
				addNote(sender, args);
			}
			else if(args[0].equalsIgnoreCase("list") && sender.hasPermission("adminaid.note.read")) {
				listPlayers(sender, args);
			}
			else if(args[0].equalsIgnoreCase("read") && (sender.hasPermission("adminaid.note.read") || sender.hasPermission("adminaid.note.read.self"))) {
				readNotes(sender, args);
			}
			else if(args[0].equalsIgnoreCase("remove") && sender.hasPermission("adminaid.note.remove")) {
				removeSingleNote(sender, args);
			}
			else if(args[0].equalsIgnoreCase("removeall") && sender.hasPermission("adminaid.note.remove")) {
